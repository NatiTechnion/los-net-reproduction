...
# Create a 2x1 vertical grid just like the paper
fig, axes = plt.subplots(2, 1, figsize=(5, 8.5))
cmap = sns.color_palette("coolwarm_r", as_cmap=True)

# ==========================================
# --- TOP PLOT: Mis-7b (Cross-Dataset) ---
# ==========================================
ax_top = axes[0]
means, stds, scratch_means, row_labels, col_labels = load_matrices('cross_datasets_results_for_model', 'mistralai')
...
sns.heatmap(means, annot=False, cmap=cmap, fmt="", 
            ...
            vmin=50, vmax=95, cbar=True, 
            cbar_kws={'label': 'Test AUC'}, ax=ax_top)
...
for i in range(means.shape[0]):
    for j in range(means.shape[1]):
        ...
        # Check against Mis-7b baselines for the specific target dataset
        is_bold = m > BEST_BASELINES['Mis-7b'].get(col_labels[j], 100)
        weight = 'bold' if is_bold else 'normal'
        
        text = f"{m:.2f}{asterisk}\n$\pm$ {s:.2f}"
        color = 'white' if m > 78 else 'black'
        ax_top.text(j + 0.5, i + 0.5, text, ha='center', va='center', 
                color=color, fontweight=weight, fontsize=11)

ax_top.set_xlabel('Target', fontweight='bold')
ax_top.set_ylabel('Source', fontweight='bold')
ax_top.set_title('Mis-7b', fontweight='bold', fontsize=14)

# ==========================================
# --- BOTTOM PLOT: IMDB (Cross-LLM) ---
# ==========================================
ax_bot = axes[1]
means, stds, scratch_means, row_labels, col_labels = load_matrices('cross_models_results_for_dataset', 'imdb')
...
sns.heatmap(means, annot=False, cmap=cmap, fmt="", 
            xticklabels=col_labels, yticklabels=row_labels, 
            vmin=50, vmax=95, cbar=True, 
            cbar_kws={'label': 'Test AUC'}, ax=ax_bot)
...
for i in range(means.shape[0]):
    for j in range(means.shape[1]):
        ...
        # Check against specific target LLM baseline for the IMDB dataset
        is_bold = m > BEST_BASELINES.get(col_labels[j], {}).get('IMDB', 100)
        weight = 'bold' if is_bold else 'normal'
        
        text = f"{m:.2f}{asterisk}\n$\pm$ {s:.2f}"
        color = 'white' if m > 78 else 'black'
        ax_bot.text(j + 0.5, i + 0.5, text, ha='center', va='center', 
                color=color, fontweight=weight, fontsize=11)

ax_bot.set_xlabel('Target', fontweight='bold')
ax_bot.set_ylabel('Source', fontweight='bold')
ax_bot.set_title('IMDB', fontweight='bold', fontsize=14)

plt.tight_layout()
plt.savefig("figure2_reproduction.png", dpi=300)
print("\nSaved figure2_reproduction.png")