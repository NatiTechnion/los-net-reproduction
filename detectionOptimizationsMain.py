...
def train_one_epoch(model, dataloader, criterion, optimizer, scheduler, device, input_type='LOS'):
    ...
    for batch in tqdm(dataloader, desc="Training Progress",disable=True):
...
def evaluate(model, dataloader, criterion, device, desc="Validation", input_type='LOS'):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"{desc} Progress",disable=True):
...
def measure_detection_time(model, dataloader, device, input_type='LOS'):
    """Measures single-example inference latency (Detection Time) for Table 7."""
    import time
    import numpy as np
    
    # 1. UNLEASH THE H200 TENSOR CORES
    ...
    # 2. USE STRICT INFERENCE MODE (No compiler, just raw PyTorch)
    with torch.inference_mode():
        for _ in range(10):
            if input_type == 'LOS':
                sorted_TDS, norm_ATP, ATP_R, _ = warmup_batch
                _ = model(sorted_TDS.to(device), norm_ATP.to(device), ATP_R.to(device))
    
    torch.cuda.synchronize()

    print("Measuring detection time over the test set...")
    with torch.inference_mode():
        for batch in tqdm(dataloader, desc="Latency Profiling"):
            if input_type == 'LOS':
                sorted_TDS_normalized, normalized_ATP, ATP_R, labels = batch
                batch_size = labels.size(0)
                
                sorted_TDS_normalized = sorted_TDS_normalized.to(device)
                normalized_ATP = normalized_ATP.to(device)
                ATP_R = ATP_R.to(device)
                
                torch.cuda.synchronize()
                start_time = time.perf_counter()
                
                _ = model(sorted_TDS_normalized, normalized_ATP, ATP_R)
                
                torch.cuda.synchronize()
                end_time = time.perf_counter()
                
                per_sample_time = (end_time - start_time) / batch_size
                times.extend([per_sample_time] * batch_size)
                
    mean_time = np.mean(times)
    std_time = np.std(times)
    
    print(f"\n==================================================")
    print(f"Detection Time: {mean_time:.2e} ± {std_time:.2e} seconds")
    print(f"==================================================\n")
...
def train_model(logger, model, dataloader_train, dataloader_val, dataloader_test, criterion, optimizer, scheduler, args, device):
    ...
    # ---------------------------------------------------------
    # NEW CODE: Run latency profiling at the very end of training
    # ---------------------------------------------------------
    logger.info("Calculating Detection Time for Table 7...")
    print("Calculating Detection Time for Table 7...")
    mean_time, std_time = measure_detection_time(model, dataloader_test, device, input_type=args.input_type)
    
    # Save it to the W&B run summary so you can easily extract it later
    wandb.run.summary["detection_time_mean"] = mean_time
    wandb.run.summary["detection_time_std"] = std_time
    print('mean:',mean_time,'std:',std_time)
    # ---------------------------------------------------------
...
def main():
    ...
    wandb.init(project="LOS-Net-Efficiency-Optimizations", config=args)
