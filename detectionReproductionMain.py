...
def measure_detection_time(model, dataloader, device, input_type='LOS'):
    ...
    # Grab a single batch to use for the warm-up
    ...
    # Wait for the warm-up to completely finish
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Latency Profiling"):
            if input_type == 'LOS':
                ...
                # Synchronize after the pass
                torch.cuda.synchronize()
                end_time = time.perf_counter()
                
                # Calculate the time it took for a SINGLE sample
                per_sample_time = (end_time - start_time) / batch_size
                
                # Append it 'batch_size' times so Mean and Std calculate correctly
